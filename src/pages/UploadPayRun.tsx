  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (matchedTrainer) {
          if (matchedTrainer.default_hourly_rate && hourlyRate && hourlyRate !== matchedTrainer.default_hourly_rate) {
            warnings.push(`CSV rate (£${hourlyRate}) differs from default rate (£${matchedTrainer.default_hourly_rate})`);
          }
